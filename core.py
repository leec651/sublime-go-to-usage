...
def use_selected_word(view):
    line_region = view.word(view.sel()[0])
    word = view.substr(line_region)
    return word
...
def find_subject_name(view):
    ...
    the first one going upwards from the current cursor position.
    """
    return (use_selected_word(view)
        or find_subject_name_on_current_line(view, CLASS_REGEX)
    ...
    if line_split[1]:
        ...
        for ignored_beginning in IGNORED_SUFFIX:
            if after.startswith(ignored_beginning):
                # let's also return the definition
                return True
